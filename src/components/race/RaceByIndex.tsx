...
import {convertiPiediInMetri} from '../../utils/convertFootToMeters';
...
export default function RaceComponent({input}: {input: RaceIndexRequest}) {
  ...
  return (
    <>
      <ScrollView>
        <StyledLabel label="Name:" value={data?.name ?? 'race not available'} />
        <StyledLabel
          label={'Speed:'}
          value={`${
            data?.speed + ' piedi' ?? 'speed not available'
          }\n${convertiPiediInMetri(data?.speed ?? 0)} metri`}></StyledLabel>
